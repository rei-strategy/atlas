...
/**
 * SECURITY: Audit logs are read-only
 * DELETE, PUT, PATCH, and POST operations are explicitly forbidden
 */
router.delete('/:id', (req, res) => {
  res.status(403).json({
    error: 'Forbidden',
    message: 'Audit logs cannot be deleted. They are permanent records for compliance and security.'
  });
});
...
router.put('/:id', (req, res) => {
  res.status(403).json({
    error: 'Forbidden',
    message: 'Audit logs cannot be modified. They are immutable records for compliance and security.'
  });
});
...
router.post('/', (req, res) => {
  res.status(403).json({
    error: 'Forbidden',
    message: 'Audit logs cannot be manually created. They are automatically generated by the system.'
  });
});
